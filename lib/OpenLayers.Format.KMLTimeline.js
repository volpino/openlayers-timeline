...
//        this.lowerlimit = new Date().getTime() / 1000;
    ...
    parseFeature: function(node) {
        ...
        if(this.extractAttributes) {
            ...
            if (date) {
                ...
                else if ( date > this.current_date ){
                	this.current_date = date;
                }
								
                if (!this.first || this.first > date) {
                    this.first = date;
                    this.firstFeature = new OpenLayers.Feature.Vector(geometry, attributes);
                }
                if( !this.past_seconds )
                {
                	// first run
                    console.log(date + " " + attributes.when + " hidden");
                    return;
                }
                ...
                console.log(date + " " + attributes.when + " display");
            }
        }
        ...
        for(var i=0, len=children.length; i<len; ++i) {
            ...
            if(child.nodeType == 1) {
                ...
                if(grandchildren.length >= 1 && grandchildren.length <= 3) {
                    ...
                    if( grandchild.nodeType == 1 && grandchild.nodeName == 'when' ) {
                        var value = OpenLayers.Util.getXmlNodeValue(grandchild);
                        if (value) {
                            attributes.when = OpenLayers.Date.parse( value );
                        }
                    }
                } 
          }
